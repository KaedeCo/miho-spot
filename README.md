...
Miho-spot 是一个多平台二游圈舆情监测与分析系统，覆盖知乎、抖音、贴吧、B站等平台。通过热搜爬取、关键词匹配、SnowNLP 本地情感分析、DeepSeek AI 增强分析，实时追踪米哈游相关舆论风向。同时提供 B站评论区增强脚本，实现实时标注与智能话术生成。v1.6.1 全面修复了桌面打包版的各项缺失功能，支持一键 EXE 运行。
...
### 🖥️ 桌面打包版 (v1.6.1)
- PyInstaller 单文件 EXE（265 MB），双击即用，无需 Python 环境
- 完整功能内置：PDF报告 + 词云 + 辩论 + 三维热力图 + 聚类分析
- 首次启动自动从内嵌资源复制种子数据（categories.json、keywords、paper/）
- 分类自动合并升级（旧文件升级不丢失用户自定义分类）
- SPA 路由完整（14 个前端页面 + 油猴脚本导出 + 辩论回放）
- B站 Cookie 策略优化（无需手动验证即可生效）
- 油猴脚本 JS 模板内嵌 + frozen 路径解析修复
...
### v1.6.1 (2026-06-07) — 桌面完整打包版

- 🖥️ 单文件 EXE 打包，双击即用，无需 Python 环境（265 MB）
- 📦 完整依赖集成：reportlab + matplotlib + numpy + scipy + wordcloud + ddgs + PIL
- 🔧 B站 Cookie 策略修复：无需验证即可生效，DIRECT 模式 -352 重试增强
- 🗂️ 分类自动合并：升级时补全缺失分类，用户自定义不被覆盖
- 🔌 油猴脚本导出修复：JS 模板内嵌 + frozen 模式路径解析
- 📄 PDF 报告修复：第三章时间轴推演末尾截断
- 🌐 SPA 路由修复：14 个前端页面完整可访问
- 🐛 首次启动种子数据自动复制（categories.json、hot_crawl.json、paper/）
