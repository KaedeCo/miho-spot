...
v1.3 是一次全面的稳定性攻坚版本，聚焦于前端组件协调、后端 API 健壮性、数据库同步策略和历史统计数据准确性四大维度的深度修复。本版本解决了 12 个关键 Bug，建立了完整的 JSON 文件→数据库→前端页面的数据流闭环。
...
### 视频分析（v1.3 新增，`/video-analysis`）
- **B站视频舆情深度挖掘** — 输入 BV 号，拉取视频评论 + 高级搜索评论双维度数据
- **KOL 影响力排行** — 按点赞数热度 / 发布时间双模式排序，展示 Top 10 评论用户及其影响力指标
- **评论词云** — 自动生成高频词云图，直观呈现评论语义分布
